...
public class SY1_SqlList1 {
	...
	public static void main(String[] args) throws Exception {
		...
		int n = sc.nextInt();
		System.out.println("请输入顺序表中个元素的值：");
		for(int i = 0;i<n;i++)
			L.insert(i,sc.nextInt());
		System.out.println("请输入带插入元素的位置i(0~curLen)：");
		int i = sc.nextInt();
		...
		L.display();
		System.out.println("请输入待删除元素的位置i(0~curLen-1)");
		i = sc.nextInt();
		L.remove(i);
		...
		if(order ==-1)
			System.out.println("此顺序表中不含有值为"+'x'+"的元素！");
		else
			System.out.println("值为"+x+"的元素在此表中的第"+order+"个位置上");
	}
